...
client.on('guildMemberAdd', function (member) {
    let embed = new Discord.RichEmbed()
        .setDescription(`Yo mon gars merci de rejoindre la ville et bienvenue ` + member.user.username + ' !! IL EST OBLIGATOIRE DE LIRE LE REGLEMENT ET DE L APPLIQUER AINSI QUE DE METTRE UN PRENOM+NOM RP et s il te plaît invite du monde et surtout bienvenue sur le serveur de ' + member.guild.name)
        .setFooter('Nous sommes désormais ' + member.guild.memberCount)
    member.guild.channels.get('667748890384138260').send(embed)
    member.addRole('667761542950944798')
}) 

client.on('guildMemberRemove', function (member) {
    let embed = new Discord.RichEmbed()
        .setDescription('Oh Non** ' + member.user.username + '** a quitter la ville :disappointed_relieved:On se Resésit il risque de regretter sachant que on a un très bon serveur !')
        .setFooter('Nous sommes désormais ' + member.guild.memberCount)
    member.guild.channels.get('667748647617691668').send(embed)
})

client.on('message', function (message) {
    if (message.content === prefix + "tu fais quoi ?") {
        message.channel.sendMessage('Je surveille les joueurs de mon serveur Panama RP et toi tu fais quoi?!')
...
client.on('message', function (message) {
    if (message.content === prefix + "Salut") {
        message.channel.sendMessage('Salut comment va tu?')
